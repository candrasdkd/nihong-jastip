...
export const RenderCargo = ({ indoToJpn, jpnToIndo }) => {
  ...
  const renderGhostRows = (currentCount: number) => {
    ...
    return Array.from({ length: slotsNeeded }).map((_, i) => (
      <div
        key={`ghost-${i}`}
        className={`flex justify-between items-center py-[1px] opacity-30 select-none grayscale`}
      >
        <div className="flex items-center gap-2">
          <span className="text-[9px] text-slate-400 font-mono w-3">--</span>
          <span
            className={`font-mono font-bold text-slate-500 uppercase ${style.rowFont}`}
          >
            COMING SOON
          </span>
        </div>
        ...
        <span
          className={`font-bold text-slate-400 border border-slate-300 bg-slate-100 rounded-sm ${style.badgeFont}`}
        >
          TBA
        </span>
      </div>
      ...
      <div className="bg-white w-full h-full rounded-lg shadow-xl overflow-hidden flex flex-col border-2 border-slate-800">
        ...
        <div className="bg-slate-800 text-white flex justify-between items-center shrink-0 p-3 relative overflow-hidden">
          <div className="z-10 flex flex-col">
            ...
            <div className="flex items-center gap-2">
              <h1 className="font-black tracking-tighter text-lg uppercase italic leading-none">
                NIHONG
              </h1>
              <div className="bg-yellow-400 text-black text-[9px] font-bold px-1 rounded-sm transform -skew-x-12">
                JASTIP
              </div>
            </div>
          </div>
        ...
        <div className="flex-1 flex flex-col bg-slate-50 relative min-h-0">
          ...
          {hasOutbound && (
            <div
              className={`flex-1 flex flex-col border-b-2 border-dashed border-slate-300 min-h-0 ${style.padding}`}
            >
              {/* Section Header */}
              <div className="flex items-center justify-between mb-1 pb-1 border-b border-slate-200 shrink-0">
                <div className="flex items-center gap-2">
                  <span className="bg-slate-200 text-slate-700 text-[9px] font-bold px-1 py-0.5 rounded-sm">
                    IDN
                  </span>
                  <span className="text-slate-400 text-[8px] font-bold">
                    TO
                  </span>
                  <span className="bg-slate-800 text-white text-[9px] font-bold px-1 py-0.5 rounded-sm">
                    JPN
                  </span>
                </div>
                <span className="text-[9px] text-slate-400 font-bold font-mono tracking-tight">
                  PRIORITY
                </span>
              </div>
              ...
              <div
                className={`flex-1 flex flex-col ${indoToJpn.length < 5 ? "justify-start gap-1" : "justify-between"}`}
              >
                {indoToJpn.map((item, i) => (
                  <div
                    key={i}
                    className="flex justify-between items-center py-[1px]"
                  >
                    <div className="flex items-center gap-2">
                      <span className="text-[9px] text-slate-400 font-mono w-3">
                        {i + 1}
                      </span>
                      <span
                        className={`font-mono font-bold text-slate-700 uppercase ${style.rowFont}`}
                      >
                        {item.date}
                      </span>
                    </div>
                    ...
                    {item.status === "AVAILABLE" ? (
                      <span
                        className={`font-bold text-blue-700 bg-blue-100 border border-blue-200 rounded-sm ${style.badgeFont}`}
                      >
                        {item.value}KG
                      </span>
                    ) : item.status === "LIMITED" ? (
                      <span
                        className={`font-bold text-amber-700 bg-amber-100 border border-amber-200 rounded-sm ${style.badgeFont}`}
                      >
                        LIMITED
                      </span>
                    ) : (
                      <span className="text-slate-300 text-[9px] decoration-2 line-through font-bold">
                        FULL
                      </span>
                    )}
                  </div>
            ...
            <div className={`flex-1 flex flex-col min-h-0 ${style.padding}`}>
              ...
              <div className="flex items-center justify-between mb-1 pb-1 border-b border-slate-200 shrink-0">
                <div className="flex items-center gap-2">
                  ...
                  <span className="bg-orange-100 text-orange-700 text-[9px] font-bold px-1 py-0.5 rounded-sm">
                    IDN
                  </span>
                </div>
                <span className="text-[9px] text-slate-400 font-bold font-mono tracking-tight">
              ...
              {/* List Items (Adaptive Layout) */}
              <div
                className={`flex-1 flex flex-col ${jpnToIndo.length < 5 ? "justify-start gap-1" : "justify-between"}`}
              >
        ...
        <div className="bg-white border-t-2 border-slate-800 p-2 flex justify-between items-center shrink-0">
          <div className="flex flex-col">
            ...
            <span className="text-[10px] font-black text-slate-800 font-mono tracking-tighter">
              NJ-{new Date().getFullYear()}-EXP
            </span>
          </div>
          {/* Barcode yang responsive */}
          <div className="opacity-90">
            <SVGBarcode
              className={`text-slate-900 ${isDense ? "h-6 w-24" : "h-8 w-28"}`}
            />
          </div>
        </div>
      </div>

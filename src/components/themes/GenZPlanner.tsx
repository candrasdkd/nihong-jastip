...
export const RenderGenZPlanner = ({ indoToJpn, jpnToIndo }) => {
  ...
  const renderGhostSlots = (currentCount: number) => {
    ...
    return Array.from({ length: slotsNeeded }).map((_, i) => (
      <div
        key={`ghost-${i}`}
        className="flex items-center justify-between py-[2px] opacity-30 select-none grayscale"
      >
        <div className="flex items-center gap-2">
          <div className="w-1.5 h-1.5 bg-slate-300 rounded-full animate-pulse"></div>
          <span
            className={`font-bold font-mono text-slate-400 tracking-tight blur-[1px] ${style.dateFont}`}
          >
            XX-XXX
          </span>
        </div>
        ...
        <div
          className={`bg-slate-100 text-slate-400 border border-slate-200 rounded-md font-bold text-[8px] px-2 py-0.5`}
        >
          COMING SOON
        </div>
      </div>
  ...
  return (
    <div className="relative z-10 w-full h-full font-sans text-slate-800 overflow-hidden p-4 flex justify-center items-center">
      ...
      {/* --- Main Card --- */}
      <div
        className={`relative w-full h-full bg-white/60 backdrop-blur-3xl border border-white/60 shadow-[0_20px_40px_-10px_rgba(0,0,0,0.1)] rounded-[2rem] flex flex-col overflow-hidden ring-1 ring-white/80 ${style.padding}`}
      >
        {/* === HEADER === */}
        <div
          className={`relative z-20 flex justify-between items-start ${style.headerGap} shrink-0`}
        >
          <div className="flex flex-col relative leading-[0.85]">
            ...
            <span className="text-[10px] font-bold tracking-[0.4em] text-slate-400 uppercase ml-1 mb-1">
              Schedule
            </span>
            <h1
              className={`font-black tracking-tighter text-slate-900 ${style.titleSize}`}
            >
              NIHONG
            </h1>
            <h1
              className={`font-black tracking-tighter text-transparent bg-clip-text bg-gradient-to-r from-blue-600 via-purple-600 to-pink-500 ${style.titleSize}`}
            >
              JASTIP.
            </h1>
          </div>
        ...
        {/* === BODY: ADAPTIVE CARDS === */}
        <div
          className={`flex-1 flex flex-col overflow-hidden min-h-0 ${style.cardGap}`}
        >
          {/* --- Card 1: OUTBOUND --- */}
          {hasOutbound && (
            <div className="flex-1 flex flex-col bg-white/50 rounded-2xl border border-white p-2 relative overflow-hidden group hover:bg-white/70 transition-colors min-h-0">
              <div className="flex items-center justify-between mb-2 px-1 shrink-0">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="font-bold text-slate-700 text-[10px] tracking-wider uppercase">
                    Jakarta <span className="text-slate-300">/</span> Osaka
                  </span>
                </div>
              ...
              <div
                className={`flex-1 flex flex-col overflow-hidden px-1 ${indoToJpn.length < 5 ? "justify-start gap-1.5" : "justify-between"}`}
              >
                {indoToJpn.map((item) => (
                  <div
                    key={item.id}
                  ...
                  >
                    <span
                      className={`font-bold font-mono text-slate-600 tracking-tight ${style.dateFont}`}
                    >
                      {item.date}
                    </span>
                    ...
                    {item.status === "AVAILABLE" ? (
                      <div
                        className={`bg-blue-50 text-blue-600 border border-blue-100 rounded-md font-bold shadow-sm ${style.badgeFont}`}
                      >
                        {item.value}kg
                      </div>
                    ) : item.status === "LIMITED" ? (
                      <div
                        className={`bg-amber-50 text-amber-600 border border-amber-100 rounded-md font-bold ${style.badgeFont}`}
                      >
                        Low
                      </div>
                    ) : (
                      <div
                        className={`text-slate-300 line-through decoration-1 ${style.badgeFont}`}
                      >
                        Sold
                      </div>
                    )}
            ...
            <div className="flex-1 flex flex-col bg-white/50 rounded-2xl border border-white p-2 relative overflow-hidden group hover:bg-white/70 transition-colors min-h-0">
              <div className="flex items-center justify-between mb-2 px-1 shrink-0">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="font-bold text-slate-700 text-[10px] tracking-wider uppercase">
                    Osaka <span className="text-slate-300">/</span> Jakarta
                  </span>
                </div>
              ...
              <div
                className={`flex-1 flex flex-col overflow-hidden px-1 ${jpnToIndo.length < 5 ? "justify-start gap-1.5" : "justify-between"}`}
              >
                {jpnToIndo.map((item) => (
                  <div
                    key={item.id}
                  ...
                  >
                    ...
                    {item.status === "AVAILABLE" ? (
                      <div
                        className={`bg-purple-50 text-purple-600 border border-purple-100 rounded-md font-bold shadow-sm ${style.badgeFont}`}
                      >
                        {item.value}kg
                      </div>
                    ) : item.status === "LIMITED" ? (

...
export const RenderBoardingPass = ({ indoToJpn, jpnToIndo }) => {
  ...
  const renderFillerSlots = (currentCount: number) => {
    ...
    return Array.from({ length: slotsNeeded }).map((_, i) => (
      <div
        key={`empty-${i}`}
        className="flex items-center justify-between py-1.5 opacity-40 group"
      >
        <div className="flex items-center gap-2">
          <span className="text-[9px] text-slate-300 font-mono w-4 text-center">
            --
          </span>
          {/* Kotak Putus-Putus */}
          <div className="border-2 border-dashed border-slate-300 rounded px-2 py-0.5 bg-slate-50">
            <span
              className={`font-mono font-bold text-slate-400 text-[10px] uppercase tracking-wider`}
            >
              COMING SOON
            </span>
          </div>
        ...
        <span
          className={`font-bold text-slate-300 border border-slate-200 bg-slate-50 rounded-sm ${style.badgeFont}`}
        >
          TBA
        </span>
      </div>
  ...
  return (
    <div className="relative z-10 w-full h-full font-sans text-slate-800 p-2">
      <div className="w-full h-full bg-white rounded-xl shadow-xl overflow-hidden flex flex-col border border-slate-200 relative">
        ...
        {/* --- HEADER --- */}
        <div
          className={`bg-blue-700 text-white flex justify-between items-center shrink-0 z-20 shadow-md relative overflow-hidden ${style.header}`}
        >
          <div className="flex flex-col z-10">
            <div className="flex items-center gap-1 mb-0.5">
              <span className="bg-white/20 px-1 rounded text-[8px] font-bold tracking-wider">
                INTL
              </span>
              <span className="text-blue-200 font-bold tracking-[0.2em] text-[8px] uppercase">
                Logistics
              </span>
            </div>
            <h1
              className={`font-black tracking-widest leading-none italic uppercase drop-shadow-sm ${style.titleSize}`}
            >
          ...
          <div className="z-10 text-right">
            <div
              className={`bg-blue-800/50 border border-blue-400/30 backdrop-blur-md rounded flex flex-col items-center justify-center ${isDense ? "px-2 py-0.5" : "px-3 py-1"}`}
            >
              <span className="text-[8px] text-blue-200 font-mono uppercase">
                CLASS
              </span>
              <span className="font-bold text-white tracking-widest text-[10px]">
                PRIORITY
              </span>
            </div>
          </div>
        ...
        <div className="flex-1 flex flex-col relative z-10 min-h-0">
          ...
          {hasOutbound && (
            <div
              className={`flex-1 flex flex-col min-h-0 ${style.sectionPad} relative`}
            >
              <div className="flex items-center justify-between border-b-2 border-slate-200 shrink-0 mb-2 pb-1 relative z-10">
                <div className="flex items-center gap-2 text-slate-700">
                  <div className="bg-slate-100 border border-slate-300 px-1.5 rounded text-xs font-black tracking-tighter">
                    JKT
                  </div>
                ...
                <span className="text-[9px] text-slate-400 font-bold uppercase tracking-widest bg-slate-50 px-1 rounded">
                  Outbound
                </span>
              </div>

              <div
                className={`flex-1 flex flex-col overflow-hidden relative z-10 ${indoToJpn.length < 5 ? "justify-start gap-1" : "justify-between"}`}
              >
                {indoToJpn.map((item, i) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <span className="text-[9px] font-mono font-bold text-slate-400 w-3">
                        {i + 1}
                      </span>
                      <span
                        className={`font-mono font-bold text-slate-700 ${style.itemFont}`}
                      >
                        {item.date}
                      </span>
                    </div>
                    ...
                    {item.status === "AVAILABLE" ? (
                      <span
                        className={`font-bold bg-blue-50 text-blue-700 rounded border border-blue-200 shadow-sm ${style.badgeFont}`}
                      >
                        OPEN {item.value}KG
                      </span>
                    ) : item.status === "LIMITED" ? (
                      <span
                        className={`font-bold bg-amber-50 text-amber-700 rounded border border-amber-200 ${style.badgeFont}`}
                      >
                        LIMITED
                      </span>
                    ) : (
                      <span className="text-slate-300 text-[8px] line-through font-bold decoration-2">
                        SOLD OUT
                      </span>
                    )}
                  </div>
          ...
          {hasInbound && (
            ...
            >
              ...
              <div
                className={`flex-1 flex flex-col overflow-hidden relative z-10 ${jpnToIndo.length < 5 ? "justify-start gap-1" : "justify-between"}`}
              >
        ...
        {/* --- FOOTER: MORE INFO --- */}
        <div
          className={`bg-slate-50 border-t border-slate-200 flex justify-between items-center shrink-0 z-10 ${isDense ? "p-2" : "p-3"}`}
        >
          ...
          <div className="opacity-80">
            <SVGBarcode
              className={`text-slate-800 ${isDense ? "h-6 w-20" : "h-8 w-28"}`}
            />
          </div>
        </div>
      </div>

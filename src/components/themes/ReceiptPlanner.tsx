...
export const RenderReceipt = ({ indoToJpn, jpnToIndo }) => {
    ...
    const renderGhostRows = (currentCount: number) => {
        ...
        return Array.from({ length: slotsNeeded }).map((_, i) => (
            <div key={`ghost-${i}`} className={`flex justify-between items-center ${style.fontSize} opacity-25 select-none grayscale`}>
        ...
        <div className="relative z-10 w-full h-full font-mono text-zinc-800 p-2 sm:p-4 flex items-center justify-center">
            ...
            <div className="relative w-full h-full bg-[#fffcf5] shadow-xl flex flex-col my-2 max-w-sm mx-auto">
                ...
                <div className={`text-center flex flex-col items-center border-b-2 border-dashed border-zinc-300 mx-3 ${style.padding} shrink-0`}>
                    ...
                    <p className="text-[10px] uppercase tracking-[0.2em] text-zinc-500">Official Receipt</p>
                </div>
                    ...
                    {hasOutbound && (
                        <div className="flex-1 flex flex-col min-h-0">
                            <div className="flex justify-between items-end border-b border-zinc-300 pb-1 mb-1 shrink-0">
                                ...
                                <span className="text-[10px] font-bold text-zinc-500">JKT &gt; OSAKA</span>
                            </div>
                            
                            {/* Logic: Jika item < 5, pakai gap biasa (start). Jika banyak, spread (between) */}
                            <div className={`flex-1 flex flex-col ${indoToJpn.length < 5 ? 'justify-start gap-1.5' : 'justify-between'} py-1`}>
                    ...
                    {hasInbound && (
                        <div className="flex-1 flex flex-col min-h-0">
                             <div className="flex justify-between items-end border-b border-zinc-300 pb-1 mb-1 shrink-0">
                                ...
                                <span className="text-[10px] font-bold text-zinc-500">OSAKA &gt; JKT</span>
                            </div>

                            <div className={`flex-1 flex flex-col ${jpnToIndo.length < 5 ? 'justify-start gap-1.5' : 'justify-between'} py-1`}>

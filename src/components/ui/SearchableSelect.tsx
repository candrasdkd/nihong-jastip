...
export default function SearchableSelect({
    ...
    const filtered = useMemo(() => {
        ...
        const arr = q ? options.filter(o => o.label.toLowerCase().includes(q)) : options;
    ...
    return (
        <div ref={wrapRef} className={`relative ${className}`}>
            {label && <label className="block mb-1 text-sm text-neutral-600">{label}</label>}
            ...
            {open && (
                <div className="absolute left-0 right-0 mt-1 z-50 rounded-xl border border-[#0a2342]/20 bg-white shadow-lg">
                    <div className="p-2 border-b border-neutral-200">
                        <input
                            ...
                            onKeyDown={(e) => {
                                ...
                                if (e.key === 'Enter') { e.preventDefault(); commitSelection(highlight); }
                    ...
                    <ul className="max-h-64 overflow-y-auto py-1">
                        {filtered.length === 0 && (
                            <li className="px-3 py-2 text-sm text-neutral-500">Tidak ada hasil</li>
                        )}
                        {filtered.map((o, idx) => {
                                ...
                                >
                                    ...
                                    {selected && (
                                        <svg width="18" height="18" viewBox="0 0 20 20" aria-hidden>
                                            <path d="M5 10.5l3 3 7-7" stroke="currentColor" strokeWidth="1.8" fill="none" strokeLinecap="round" />
                                        </svg>
                                    )}
                                </li>
                            );

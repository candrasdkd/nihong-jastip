...
const getStatusBadge = (status: string) => {
  ...
  if (s.includes('selesai') || s.includes('lunas') || s.includes('diterima')) {
    return { text: 'LUNAS', color: 'text-emerald-600 border-emerald-600 bg-emerald-50' };
  } else if (s.includes('batal')) {
    return { text: 'DIBATALKAN', color: 'text-red-600 border-red-600 bg-red-50' };
  } else {
    return { text: 'TAGIHAN', color: 'text-slate-600 border-slate-600 bg-slate-50' };
  }
};
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between items-start border-b-2 border-slate-900 pb-6 mb-6">
        <div>
          <h1 className="text-3xl font-extrabold tracking-tight text-slate-900 uppercase" style={{ letterSpacing: '2px' }}>INVOICE</h1>
          <div className="mt-4 text-sm text-slate-600 leading-relaxed font-sans">
            <p className="font-bold text-slate-900 text-base">Nihong Jastip</p>
        ...
        <div className="text-right">
          ...
          <div className="space-y-1 font-sans">
            <div className="flex justify-end gap-4 text-sm">
              ...
              <span className="font-mono font-medium">INV/{(order as any).no || 'NEW'}/{new Date().getFullYear()}</span>
            </div>
            <div className="flex justify-end gap-4 text-sm">
              <span className="text-slate-500">Tanggal:</span>
              <span className="font-medium">{order.tanggal || new Date().toLocaleDateString('id-ID')}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex justify-between mb-8">
        <div className="w-1/2 pr-4">
          <h3 className="text-xs uppercase tracking-wider font-bold text-slate-400 mb-2">DITAGIHKAN KEPADA</h3>
          <div className="text-base font-bold text-slate-900">{order.namaPelanggan}</div>
          {customer?.alamat && <div className="text-sm text-slate-600 mt-1 max-w-xs leading-snug">{customer.alamat}</div>}
        ...
        <table className="w-full text-sm border-collapse">
          <thead>
            <tr className="border-y-2 border-slate-900">
              <th className="py-3 text-left font-bold text-slate-900 uppercase tracking-wider text-xs w-[40%]">Deskripsi</th>
              <th className="py-3 text-left font-bold text-slate-900 uppercase tracking-wider text-xs">Kategori</th>
              <th className="py-3 text-right font-bold text-slate-900 uppercase tracking-wider text-xs">Berat (Kg)</th>
              <th className="py-3 text-right font-bold text-slate-900 uppercase tracking-wider text-xs">Total (IDR)</th>
            </tr>
          </thead>
              ...
              return (
                <tr key={idx}>
                  <td className="py-3 pr-4 align-top">
                    ...
                    {item.catatan && <div className="text-xs text-slate-500 italic mt-1">{item.catatan}</div>}
                  </td>
                  <td className="py-3 text-slate-600 align-top">{item.kategori || '-'}</td>
                  <td className="py-3 text-right text-slate-600 font-mono align-top">{d.kg}</td>
                  <td className="py-3 text-right font-mono font-bold text-slate-900 align-top">
                    {formatCurrency(d.lineTotal, d.currency).replace('IDR', '').trim()}
                  </td>
                </tr>
              );
            })}
      ...
      <div className="flex flex-row justify-between items-start pt-6 border-t border-slate-300">
        ...
        <div className="w-[55%] pr-8">
          ...
          <div className="text-sm text-slate-600 mb-4">
            <p className="mb-3">Mohon transfer ke salah satu rekening berikut:</p>
            <div className="space-y-3">
              {BANK_ACCOUNTS.map((acc, idx) => (
                <div key={idx} className="bg-slate-50 p-3 rounded border border-slate-200 shadow-sm break-inside-avoid">
                  <div className="flex justify-between items-center text-xs font-bold text-slate-800">
                    <span className="text-sm">{acc.bank}</span>
                    <span className="font-mono text-sm tracking-wide">{acc.number}</span>
                  </div>
...
export function InvoiceModal({
  ...
  const paperProps = { order, items, customer, totals, grandTotal, badge, unitPrice };

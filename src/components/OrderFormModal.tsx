...
export function OrderFormModal({
  ...
  // State Mata Uang
  const [currency, setCurrency] = useState<'IDR' | 'JPY'>((initial as any)?.currency || 'IDR');
  ...
  const [hargaOngkir, setHargaOngkir] = useState<number>((initial as any)?.hargaOngkir ?? 0);
  ...
  return (
    <Modal onClose={loading ? () => { } : onClose} title={initial ? 'Edit Pesanan' : 'Tambah Pesanan'} size="5xl" contentClassName="w-full">
      ...
      <div className="px-2 mb-4">
        <label className="block mb-2 text-sm font-medium text-neutral-700">Mata Uang Transaksi</label>
        ...
        <form id="order-form" onSubmit={submit} className="grid grid-cols-1 gap-6">
          <fieldset disabled={loading} className={loading ? 'opacity-70' : ''}>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <Input label="Nama Barang" value={namaBarang} onChange={(e) => setNamaBarang(e.target.value)} required />
              <div className="lg:col-span-1">
                <label className="block mb-1 text-sm text-neutral-600">Kategori</label>
                <Select value={kategori} onChange={(e) => setKategori(e.target.value)}>
                  {CATEGORY_OPTIONS.map(k => <option key={k} value={k}>{k}</option>)}
                </Select>
              </div>
              <Input label="Tanggal" type="date" value={tanggal} onChange={(e) => setTanggal(e.target.value)} required />
              <div className="lg:col-span-1">
                <SearchableSelect label="Nama Pelanggan *" value={namaPelanggan} onChange={setNamaPelanggan} options={customerOptions} disabled={loading} />
              </div>
              <Input label="Berat (Kg)" type="number" step="0.01" value={toStr(jumlahKg)} onChange={(e) => setJumlahKg(num(e.target.value))} required />
              <Input label="Lokasi Pengiriman" value={pengiriman} onChange={(e) => setPengiriman(e.target.value)} className="lg:col-span-2" />
              <div className="lg:col-span-1">
            ...
            {/* SEKSI HARGA */}
            <div className={`mt-6 rounded-2xl border p-4 bg-white/50 space-y-4 ${currency === 'JPY' ? 'border-red-100' : 'border-orange-100'}`}>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <RupiahInput currency={currency} label={`Jastip Asli (${currency})`} value={hargaJastipManual} onChange={setHargaJastipManual} />
                <RupiahInput currency={currency} label={`Jastip Nihong (${currency})`} value={hargaJastipMarkup} onChange={setHargaJastipMarkup} />
                <RupiahInput currency={currency} label={`Ongkir Asli (${currency})`} value={hargaOngkir} onChange={setHargaOngkir} />
                <RupiahInput currency={currency} label={`Ongkir Nihong (${currency})`} value={hargaOngkirMarkup} onChange={setHargaOngkirMarkup} />
              </div>

              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 border-t pt-4">
                <div>
                  <label className="block mb-1 text-sm font-medium text-neutral-600">Total Pembayaran</label>
                ...
                <div>
                  <label className="block mb-1 text-sm font-medium text-neutral-600">Total Keuntungan</label>
                  <div className={`px-3 py-2 rounded-xl border font-bold text-lg ${profitNegative ? "border-red-300 bg-red-50 text-red-700" : "border-green-200 bg-green-50 text-green-700"}`}>

...
function DashboardView({
  ...
  const { monthlyData, kpi, recentOrders } = useMemo(() => {
    ...
    const _monthly = Array.from(map.values()).sort((a, b) =>
      a.key.localeCompare(b.key),
    );
    const activeCount = _filtered.filter(
    ...
    const lastMonthRev = _monthly.length
      ? _monthly[_monthly.length - 1].total
      : 0;
    ...
    return {
      ...
      recentOrders: _filtered.slice(0, 5),
      kpi: {
          ...
          <Card className="p-6 bg-white rounded-3xl shadow-sm border border-slate-100 h-full flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto pr-1 space-y-4 scrollbar-thin scrollbar-thumb-slate-200">
              {recentOrders.map((order: any, idx) => (
                <div
                  key={idx}
                  className="flex items-center justify-between p-3 hover:bg-slate-50 rounded-xl transition-colors border border-transparent hover:border-slate-100 group"
                >
                  ...
                  <div className="text-right">
                    ...
                    <span
                      className={`text-[10px] px-2 py-0.5 rounded-full ${DONE_SET.has(order.status) ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"}`}
                    >
                      {order.status}
                    </span>
                  </div>
...
function ApplicationView({
  ...
  const FEATURES = [
    {
      id: "generator",
      title: "Generator Story",
      desc: "Buat story IG otomatis.",
      icon: Settings,
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {FEATURES.map((feature) => (
          ...
          >
            <div
              className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-4 ${feature.bg} ${feature.color} group-hover:scale-110 transition-transform duration-200`}
            >

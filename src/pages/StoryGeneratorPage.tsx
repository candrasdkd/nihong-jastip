...
const FlightBoardGenerator = () => {
  ...
  const [activeRoute, setActiveRoute] = useState<"INDO-JPN" | "JPN-INDO">(
    "INDO-JPN",
  );
  ...
  const [batchMonth, setBatchMonth] = useState(
    LONG_MONTHS_LABEL[new Date().getMonth()],
  );
  ...
  // --- EFFECT ---
  useEffect(() => {
    localStorage.setItem("schedule_indo_jpn", JSON.stringify(indoToJpn));
  }, [indoToJpn]);
  useEffect(() => {
    localStorage.setItem("schedule_jpn_indo", JSON.stringify(jpnToIndo));
  }, [jpnToIndo]);
  ...
  const handleUpdateItem = (
    ...
    const updater = (items: ScheduleItem[]) =>
      items.map((item) => {
  ...
  const handleDelete = (id: number, route: "INDO-JPN" | "JPN-INDO") => {
    if (route === "INDO-JPN")
      setIndoToJpn(indoToJpn.filter((i) => i.id !== id));
        ...
        <div className="p-5 flex flex-col gap-6 overflow-y-auto max-h-[85vh] custom-scrollbar">
          ...
          <section>
            <div className="grid grid-cols-2 gap-2">
              {[
                ...
                >
                  <theme.icon
                    className={`w-4 h-4 ${currentTheme === theme.id ? theme.color.replace("bg-", "text-") : ""}`}
                  />
          ...
          <section className="space-y-4">
            ...
            <div className="bg-slate-900 p-1 rounded-lg flex border border-slate-800">
              <button
                onClick={() => setActiveRoute("INDO-JPN")}
                className={`flex-1 py-2 text-xs font-bold rounded-md flex items-center justify-center gap-2 transition-all ${activeRoute === "INDO-JPN" ? "bg-slate-700 text-white shadow-sm" : "text-slate-500 hover:text-slate-300"}`}
              >
                <PlaneTakeoff className="w-3 h-3" /> ID ➝ JP
              </button>
              <button
                onClick={() => setActiveRoute("JPN-INDO")}
                className={`flex-1 py-2 text-xs font-bold rounded-md flex items-center justify-center gap-2 transition-all ${activeRoute === "JPN-INDO" ? "bg-slate-700 text-white shadow-sm" : "text-slate-500 hover:text-slate-300"}`}
              >
            ...
            <div className="flex gap-2">
              <div className="flex-1 flex gap-2">
                ...
                <button
                  onClick={handleAdd}
                  disabled={!inputDate}
                  className="bg-slate-700 hover:bg-slate-600 text-white px-3 rounded-lg font-bold"
                >
              ...
              <div className="max-h-64 overflow-y-auto p-1 space-y-1 custom-scrollbar">
                {activeList.length === 0 && (
                  <div className="text-center py-8 text-xs text-slate-600 italic">
                    No schedule. Add one or use Batch Add.
                  </div>
                )}
                {activeList.map((item) => (
                  <div
                    key={item.id}
                    className="flex gap-2 items-center bg-slate-800/50 p-1.5 rounded border border-slate-700/50 group hover:border-slate-600 transition-colors"
                  >
                    {/* Date Input */}
                    <input
                      type="text"
                    ...
                    <select
                      ...
                      className={`bg-transparent text-[10px] font-bold outline-none rounded px-1 cursor-pointer w-20 ${
                    ...
                    <button
                      onClick={() => handleDelete(item.id, activeRoute)}
                      className="text-slate-600 hover:text-red-400 p-1 rounded hover:bg-slate-700 opacity-0 group-hover:opacity-100 transition-opacity"
                    >
          ...
          <section className="pt-4 border-t border-slate-800 space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <div className="relative group">
                ...
                />
                <button
                  className={`w-full py-2.5 px-3 rounded-lg text-xs font-bold border border-dashed flex items-center justify-center gap-2 transition-all ${bgImage ? "border-green-500/50 bg-green-500/10 text-green-400" : "border-slate-600 bg-slate-800 text-slate-400 hover:bg-slate-700"}`}
                >
                  {bgImage ? (
                ...
                {bgImage && (
                  <button
                    ...
                    className="absolute -top-1 -right-1 bg-red-500 text-white rounded-full p-0.5 z-20 shadow"
                  >
                    <X className="w-3 h-3" />
                  </button>
                )}
              ...
              <button
                onClick={() => setExportScale(exportScale === 3 ? 4 : 3)}
                className={`py-2.5 px-3 rounded-lg text-xs font-bold border flex items-center justify-center gap-2 transition-all ${exportScale === 4 ? "border-purple-500/50 bg-purple-500/10 text-purple-400" : "border-slate-600 bg-slate-800 text-slate-400 hover:bg-slate-700"}`}
              >
            ...
            <button
              ...
              className="w-full py-3 bg-blue-600 hover:bg-blue-500 text-white rounded-xl font-bold shadow-lg flex items-center justify-center gap-2 transition-all active:scale-[0.98]"
            >
              {isExporting ? (
        ...
        <div className="relative shadow-[0_20px_50px_-12px_rgba(0,0,0,0.5)] rounded-[32px] p-2 bg-[#2a2a2a] border border-slate-700">
          ...
          <div className="rounded-[28px] overflow-hidden border-2 border-slate-900 bg-zinc-900">
            <div
              ...
              className="relative overflow-hidden select-none flex flex-col items-center justify-center bg-zinc-900"
            >
              <div className="absolute inset-0 z-0">
                {bgImage ? (
                  ...
                  <>
                    <img
                      src={THEME_BACKGROUNDS[currentTheme]}
                      alt="bg"
                      className={`w-full h-full object-cover transition-all duration-700 ease-in-out ${currentTheme === "NEON" ? "scale-110 blur-[2px] opacity-70" : currentTheme === "BOARDING" ? "blur-[1px] opacity-60" : currentTheme === "CARGO" ? "sepia-[0.3] opacity-50" : ""}`}
                    />
                    <div
                      className={`absolute inset-0 mix-blend-overlay transition-colors duration-700 ${currentTheme === "NEON" ? "bg-indigo-900/40" : currentTheme === "BOARDING" ? "bg-blue-500/20" : currentTheme === "CARGO" ? "bg-yellow-900/30" : ""}`}
                    ></div>
                  </>
                )}
              </div>
              <div className="relative w-full h-full z-10 transition-opacity duration-300">
                {currentTheme === "BOARDING" && (
                ...
                {currentTheme === "PASTEL" && (
                  <RenderReceipt indoToJpn={indoToJpn} jpnToIndo={jpnToIndo} />
                )}
                {currentTheme === "CARGO" && (
                  <RenderCargo indoToJpn={indoToJpn} jpnToIndo={jpnToIndo} />
                )}
              </div>
              {currentTheme !== "NEON" && (
                <div className="absolute bottom-6 font-bold text-white/20 text-[10px] tracking-[0.3em] uppercase z-10 pointer-events-none mix-blend-overlay">
                  Flight Board Generator
                </div>
              )}
            </div>
          </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-slate-400">
                    Add multiple dates for one month
                  </p>
                </div>
              </div>
              <button
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center px-1">
                  ...
                  <span className="text-[10px] text-slate-600 italic">
                    Day {batchMonth} will be auto-generated
                  </span>
                </div>
                  ...
                  >
                    ...
                    <div className="relative w-16">
                      <input
                        ...
                        value={row.day}
                        onChange={(e) =>
                          updateBatchRow(index, "day", e.target.value)
                        }
                    ...
                    <select
                      value={row.status}
                      onChange={(e) =>
                        updateBatchRow(index, "status", e.target.value)
                      }
                      className={`flex-1 bg-slate-900 border border-slate-700 rounded-lg px-2 py-2 text-xs font-bold focus:outline-none ${
                    ...
                    {row.status === "AVAILABLE" && (
                      <div className="relative w-16">
                        <input
                          ...
                          value={row.kg}
                          onChange={(e) =>
                            updateBatchRow(index, "kg", e.target.value)
                          }
                        ...
                        />
                        <span className="absolute right-2 top-2.5 text-[10px] text-slate-500">
                          kg
                        </span>
                      </div>
                    )}
